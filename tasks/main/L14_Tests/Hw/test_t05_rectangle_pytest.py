...
#
# Ваша задача - написать набор тестов для класса Rectangle, чтобы убедиться, что он работает правильно
...
def test_subtraction():
    ...
    Произведите операцию вычитания и убедитесь, что полученный прямоугольник имеет правильные значения ширины и высоты.
    """
    r = Rectangle(10, 1) - Rectangle(3, 4)
    assert r.width > 0
    assert r.height > 0
...
def test_subtraction_negative_result():
    ...
    """
    with pytest.raises(NegativeValueError):
        r = Rectangle(3, 4) - Rectangle(10, 1)
