...
class Fish(Animal):
    ...
    def depth(self):
        return "Мелководная рыб" if self.max_depth < 10 else "Глубоководная рыба" if self.max_depth > 100 else "Средневодная рыба"
...
class Mammal(Animal):
    ...
    def category(self) -> str:
        return "Малявка" if self.weight < 1 else "Гигант" if self.weight > 200 else "Обычный"


class AnimalFactory:
    @staticmethod
    def create_animal(animal_type: str, *args) -> [Animal, ValueError]:
        if animal_type == 'Bird':
            return Bird(*args)
