...
# Ширина rect4: 2
class Rectangle:
    def __init__(self, width: float, height: int = None):
        self.width = width
    ...
    def __add__(self, other: 'Rectangle'):
        # Неправильно:
        # Надо использовать тот же алгоритм вычисления ширины высоты по сложению периметров,
    ...
    def __sub__(self, other: 'Rectangle'):
        ...
        # Неправильно:
        # Надо вычислять независимо от большего и меньшего размера - по абсолютной величине
        new_height = self.height - other.height
        # Неправильно:
        # надо использовать целочисленно деление // вместо приведения int()
        new_width = int(new_perimeter / 2 - new_height)
        if new_width <= 0 or new_height <= 0:
            raise ValueError
        return Rectangle(new_width, new_height)
    ...
    # Неправильно:
    # Полная реализация методов необходима на основе базовых отличающихся друг от друга
    #  < и <=, а не < и >.
