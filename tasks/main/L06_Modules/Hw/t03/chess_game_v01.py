...
def generate_boards() -> []:
    ...
    count1 = 0
    count2 = 0
    while len(board_list) < SUCCESS_BOARDS:
        queens = set()
        # queens_num = randint(0, MAX_QUEENS)
        queens_num = MAX_QUEENS
        while len(queens) < queens_num:
            queens.add((randint(1, SIZE), randint(1, SIZE)))
        queens = tuple(queens)
        count1 += 1
        if not queens in queens_set:
            count2 += 1
            print(f'\r{count1:>15}{count1:>15}', end='')
